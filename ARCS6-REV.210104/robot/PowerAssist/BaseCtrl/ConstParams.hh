...
namespace ARCS {	// ARCS名前空間
	...
	class ConstParams {
		public:
			...
			static constexpr int SCR_VERTICAL_MAX = 44;			//!< [文字] 画面の最大高さ文字数
			static constexpr int SCR_HORIZONTAL_MAX = 158;		//!< [文字] 画面の最大幅文字数
			...
			static constexpr double DATA_END   = 35;		//!< [s] 保存終了時刻
			static constexpr double DATA_RESO  = 200e-6;		//!< [s] データの時間分解能
			...
			//   s  m  u  n	制御周期は Ts[0] ≦ Ts[1] ≦ … ≦ Ts[THREAD_MAX] になるようにすること
				    1000000,	// [ns] 制御用周期実行関数1 (スレッド1) 制御周期
				     500000,	// [ns] 制御用周期実行関数2 (スレッド2) 制御周期
			...
			static constexpr unsigned int ACTUATOR_NUM = 2;		//!< 実験装置のアクチュエータの総数
			...
			static constexpr std::array<double, ACTUATOR_MAX> ACT_FORCE_TORQUE_CONST = {
				0.544,	// [N/A]/[Nm/A]  1番 アクチュエータ
				0.544,	// [N/A]/[Nm/A]  2番 アクチュエータ
			...
			static constexpr double PLOT_TIMESPAN = DATA_END;					//!< [s] プロットの時間幅
