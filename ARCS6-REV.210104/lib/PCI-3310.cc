...
PCI3310::PCI3310(unsigned int Base0)
...
void PCI3310::SetVoltage(double Vout[MAX_CH]){
...
//! @return DACバイナリデータ
unsigned short PCI3310::VoltToDacData(double Vdac){
	// DACの仕様に従い定数を変更せよ
...
void PCI3310::Output(unsigned short DACdata[MAX_CH]){
	//------- DAC 出力
	// DAC チャネル 1
...
//! @param[in] 2バイトデータ
//! @return 上位1バイトデータ
unsigned short PCI3310::IIbyteHi(unsigned short in){
	return 0x00FF & (in >> 8);
}
...
//! @param[in] 2バイトデータ
//! @return 下位1バイトデータ
unsigned short PCI3310::IIbyteLo(unsigned short in){
	return 0x00FF & in;
}

//! @brief 汎用出力ピン2chからValを出力
//! @param[in] Val 出力バイナリデータ
void PCI3310::SetDigitalOut(unsigned short Val){
	outb(0x0003 & Val,BaseAddr0+0x1E);		//下位2bitをマスクしてValを出力
}

//! @brief 汎用入力ピン2chの値を入力
//! @return 入力バイナリデータ
unsigned short PCI3310::GetDigitalIn(void){
